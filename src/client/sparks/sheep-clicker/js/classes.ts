...
function significantCoefficient(dec: Decimal, decimalPlaces = 3) {
    ...
    // Values with dedicated suffixes (like k, mil, etc.)
        ...
        return (
            str.slice(0, preDecimalDigits) +
            "." +
            str.slice(preDecimalDigits, preDecimalDigits + decimalPlaces)
        );
    }
...
function getDecimalSuffix(dec: Decimal): string {
    ...
    if (exp3pair <= NUMBER_SUFFIXES.length) return NUMBER_SUFFIXES[exp3pair];
    else return `e${dec.e}`;
}
...
export const units = [
    new Unit(
        "Shepherd",
        "These guys can help you manage your business.",
        (lv) => Math.ceil((lv * 9 + 3) / 4) + 15,
    ),

    new Unit(
        "Shearer",
        "Shearers are trained for this task. " + "Leave it to the pros!",
        (lv) => Math.ceil((lv * 6 + 1) / 3) + Math.ceil(lv / 3),
    ),

    new Unit(
        "Knitter",
        "The kind knitters down the street have " +
    ...
    new Unit(
        "Babysitter",
        "You're too busy to watch ALL the sheep. " +
...
upgradeDB.forEach((u, i) => {
    const upgradeObj = new Upgrade(i, u.name, u.desc, u.cost, u.upgrade);
});
